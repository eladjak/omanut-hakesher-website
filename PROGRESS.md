...
## 2026-07-10 — fleet-sweep: תיקון קישורים-מתים בדומיין + ניקוי (ענף feat/fleet-sweep-2026-07-10)
**הבאג שנתפס (P0, חי בפרוד):** `omanut-hakesher.co.il` הוא NXDOMAIN (מעולם לא נרשם!) — וכל הקישורים הפונקציונליים הצביעו עליו: (1) מיילי-הברוכים-הבאים של כל 7 מגנטי-הלידים שלחו כפתור-הורדה מת לכל ליד שנרשם; (2) שיתופי-הוואטסאפ ב-6 כלי-הספר (רמזור-סיכויים, גלגל-רגשות, מכתב-לעתיד, מי-אני, כימיה, ציון-חיבור) שיתפו קישור מת; (3) שני מסלולי-התשלום (`/api/checkout` + `/api/sumit/checkout`) היו מפנים קונה אחרי תשלום לדומיין מת ברגע ש-SUMIT_* ייכנס ל-Vercel.
**התיקון:** מקור-אמת חדש `src/lib/site-url.ts` — `CANONICAL_SITE_URL` (SEO, נשאר placeholder עד הסטת-DNS) מופרד מ-`FUNCTIONAL_BASE_URL` (קישורים שחייבים לעבוד היום: env override → fallback ל-vercel.app החי, עם שסתום נגד הדומיין המת). `lead-magnets.ts` עבר לנתיבים יחסיים שנפתרים בזמן-שליחה; שני ה-checkout והקומפוננטות עברו לקבוע המרכזי. בהסטת-DNS: env אחד ב-Vercel מחליף הכל (עודכן ב-runbook).
**ניקוי:** נמחק `src/lib/supabase.ts` (לא-מיובא מאף מקום) + הוסרו `@supabase/supabase-js`, `@tanstack/react-table` (deps מתים; lockfile diff נקי, ‑106 שורות).
**שערים:** tsc ✓0 · build ✓ (כל הראוטים) · grep מאמת שאף קישור פונקציונלי לא מצביע לדומיין המת · אימות חי אחרי deploy.
**נותר ידוע:** 43 npm-audit vulns — כולן בעץ של Sanity (vite/hono/ws/undici), CMS שמעולם לא חוּוט לתוכן (הבלוג ב-`blog-posts.ts`) — מועמד-הסרה #1 בתוכנית-השיפורים `Documents/reports/fleet-sweep/omanut-hakesher.md`.
